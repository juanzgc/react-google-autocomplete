...
function usePlacesWidget(props) {
  ...
  var languageQueryParam = language ? "&language=".concat(language) : ""; // const librariesDefault = libraries ? `${libraries.toString()}` : "places";

  var googleMapsScriptUrl = "".concat(googleMapsScriptBaseUrl, "?libraries=geometry,places&key=").concat(apiKey).concat(languageQueryParam);
